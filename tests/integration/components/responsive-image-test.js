...
describe('Integration: Responsive Image Component', function () {
  ...
  it('it renders lazy by default', async function () {
    ...
    expect(find('img').getAttribute('data-srcset')).to.have.string(
  ...
  it('it skip lazy attributes if lazy is disabled', async function () {
    await render(
      hbs`{{responsive-image lazy=false image="lazy.jpg" size="100"}}`
    );
  ...
  it('it supports lqip if configured', async function () {
    ...
    expect(find('img').getAttribute('src')).to.be.equal(
      '/assets/images/responsive/remote50w.jpg'
    );
    this.set('image', 'inline.jpg');
    expect(find('img').getAttribute('src')).to.have.string(
